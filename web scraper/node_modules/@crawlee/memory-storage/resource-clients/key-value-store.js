...
class KeyValueStoreClient extends base_client_1.BaseClient {
    ...
    listKeys(options = {}) {
        const { limit, exclusiveStartKey, prefix } = shapeshift_1.s
            .object({
            limit: shapeshift_1.s.number.greaterThan(0).optional,
            exclusiveStartKey: shapeshift_1.s.string.optional,
            collection: shapeshift_1.s.string.optional, // This is ignored, but kept for validation consistency with API client.
            prefix: shapeshift_1.s.string.optional,
        })
            .parse(options);
        return (0, utils_1.createKeyList)((pageExclusiveStartKey) => this.listKeysPage({
            limit: limit ?? consts_1.DEFAULT_API_PARAM_LIMIT,
    ...
    keys(options = {}) {
        const { limit, exclusiveStartKey, prefix } = shapeshift_1.s
            ...
            limit: shapeshift_1.s.number.greaterThan(0).optional,
            exclusiveStartKey: shapeshift_1.s.string.optional,
            collection: shapeshift_1.s.string.optional,
            prefix: shapeshift_1.s.string.optional,
        })
            .parse(options);
        return (0, utils_1.createKeyStringList)((pageExclusiveStartKey) => this.listKeysPage({
            limit: limit ?? consts_1.DEFAULT_API_PARAM_LIMIT,
            exclusiveStartKey: pageExclusiveStartKey,
            prefix,
        }), { exclusiveStartKey, limit });
    ...
    values(options = {}) {
        ...
        const getFirstPageValues = async () => {
            ...
            const limiter = (0, p_limit_1.default)(GET_RECORD_CONCURRENCY);
        ...
        return (0, utils_1.createLazyIterablePromise)(getFirstPageValues, asyncGenerator);
    }
    entries(options = {}) {
        const keys = this.keys.bind(this);
        const getRecord = this.getRecord.bind(this);
        ...
        const getFirstPageEntries = async () => {
            ...
            const keysToFetch = limit !== undefined ? firstPageKeys.items.slice(0, limit) : firstPageKeys.items;
            const limiter = (0, p_limit_1.default)(GET_RECORD_CONCURRENCY);
            const results = await Promise.all(keysToFetch.map((item) => limiter(() => getRecord(item.key).then((record) => ({ key: item.key, record })))));
            return results
                .filter((r) => r.record !== undefined)
                .map((r) => [r.key, r.record.value]);
        };
        async function* asyncGenerator() {
            const firstPageKeys = await firstPageKeysPromise;
            let yielded = 0;
        ...
        return (0, utils_1.createLazyIterablePromise)(getFirstPageEntries, asyncGenerator);
    }
    async listKeysPage(options = {}) {
        const { limit = consts_1.DEFAULT_API_PARAM_LIMIT, exclusiveStartKey, prefix } = options;
        // Check by id
        const existingStoreById = await (0, cache_helpers_1.findOrCacheKeyValueStoreByPossibleId)(this.client, this.name ?? this.id);
        if (!existingStoreById) {
            this.throwOnNonExisting(consts_1.StorageTypes.KeyValueStore);
        ...
        if (exclusiveStartKey) {
            ...
            if (keyPos !== -1)
                truncatedItems = filteredItems.slice(keyPos + 1);
        }
