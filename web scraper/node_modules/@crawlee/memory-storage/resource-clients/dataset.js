...
class DatasetClient extends base_client_1.BaseClient {
    ...
    listItems(options = {}) {
        const { desc, limit, offset } = shapeshift_1.s
            .object({
            ...
            .parse(options);
        return (0, utils_1.createPaginatedList)((pageOffset, pageLimit) => this.listItemsPage({
            desc,
            offset: pageOffset,
    ...
    listEntries(options = {}) {
        const { desc, limit, offset } = shapeshift_1.s
            ...
            .parse(options);
        return (0, utils_1.createPaginatedEntryList)((pageOffset, pageLimit) => this.listItemsPage({
            desc,
            offset: pageOffset,
            limit: Math.min(pageLimit, LIST_ITEMS_LIMIT),
        }), { offset, limit });
    ...
    async pushItems(items) {
        const rawItems = shapeshift_1.s
            .union(shapeshift_1.s.string, shapeshift_1.s.object({}).passthrough, shapeshift_1.s.array(shapeshift_1.s.union(shapeshift_1.s.string, shapeshift_1.s.object({}).passthrough)))
            .parse(items);
        // Check by id
        const existingStoreById = await (0, cache_helpers_1.findOrCacheDatasetByPossibleId)(this.client, this.name ?? this.id);
        if (!existingStoreById) {
