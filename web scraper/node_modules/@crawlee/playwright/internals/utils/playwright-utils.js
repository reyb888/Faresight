...
async function parseWithCheerio(page, ignoreShadowRoots = false, ignoreIframes = false) {
    ...
    if (page.frames().length > 1 && !ignoreIframes) {
        ...
        if (frames.length !== cheerioIframes.length) {
            log.warning(`parseWithCheerio: iframe count mismatch between live DOM (${frames.length}) and page snapshot (${cheerioIframes.length}). Some iframes may not be expanded.`);
        }
...
async function handleCloudflareChallenge(page, url, session, options = {}) {
    ...
    for (let i = 0; i < 10; i++) {
        await (0, utils_1.sleep)((options.preChallengeSleepSecs ?? 1) * 1000);
