...
exports.discoverValidSitemaps = discoverValidSitemaps;
...
class Sitemap {
    ...
    static async parse(sources, proxyUrl, parseSitemapOptions) {
        ...
        try {
            ...
            log_1.default.warning(`Sitemap.load: Failed to load sitemap, returning empty result. (${e})`);
            return new Sitemap([]);
        }
 ...
 */
async function* discoverValidSitemaps(urls, options = {}) {
    const { proxyUrl, timeoutMillis = 60000, signal: externalSignal, requestTimeoutMillis = 20000 } = options;
    ...
    const discoverSitemapsForDomainUrls = async function* (hostname, domainUrls) {
        ...
        try {
            const robotsFile = await robots_1.RobotsFile.find(domainUrls[0], proxyUrl, {
        ...
        if (sitemapUrl !== undefined) {
            ...
            for (const [index, result] of candidateResults.entries()) {
                ...
                if (result.status === 'fulfilled') {
                    ...
                    log_1.default.debug(`Failed to check sitemap candidate ${candidateSitemapUrl} for ${hostname}`, {
                        error: result.reason,
                    });
                }
            }
    ...
    try {
        for await (const url of (0, iterables_1.mergeAsyncIterables)(...iterables)) {
            yield url;
        }
    }
    finally {
